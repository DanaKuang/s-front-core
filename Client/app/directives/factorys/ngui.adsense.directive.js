...
define([], function () {
  ...
  // 这里的ngui.adsense是什么规则？？？ 应该是个文件名
  var adsense = angular.module('ngui.adsense', []);

  // fixme: $compile, util这几个各自作用？？？
  // $http 是 AngularJS 中的一个核心服务，用于读取远程服务器的数据。
  // 所有的应用都有一个 $rootScope，它可以作用在 ng-app 指令包含的所有 HTML 元素中。
  var adsenseFn = function ($rootScope, $http, $compile, $timeout, util, $model) {
    var defaults = { //默认配置
      tpl: '/adsense.tpl.html',
      list: [],
      totalCount: 0,
      size: 0,
      curPage: 1,
    ...
    function linkFn (scope, element, attrs) { // 这个$scope是directive的scope
      ...
      // 监视conf变化更新page，也就是监听adsenseConf，自定义指令的属性在指令的scope下。
      ...
      // 查看
      scope.viewAdsense = function (e, id) {
        scope.$emit('viewAdsense', event, {id: id})
      }
    }
    return defineObj;
  }
  ...
  adsense.directive('saAdsense', ['$rootScope', '$http', '$compile', '$timeout', 'util', 'adsenseModel', adsenseFn]);
})
